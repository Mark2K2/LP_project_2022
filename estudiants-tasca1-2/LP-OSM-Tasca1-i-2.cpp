	...
	*/
	...
	if (viejoRoble.id_element == "node")
	{
		for (int i = 0; i < viejoRoble.atributs.size(); i++)
		{
			...
			{
				lats = viejoRoble.atributs[i].second;
			}
			if (viejoRoble.atributs[i].first == "lon")
			{
				lons = viejoRoble.atributs[i].second;
			}
		}
		for (int i = 0; i < viejoRoble.fills.size(); i++)
		{
	...
	std::cout << "El restaurant " << name << " esta a aquesta posicio: " << lats << ", " << lons << std::endl;
		...
		for (int i = 0; i < camiJoaquim.fills.size(); i++)
		{
			if (camiJoaquim.fills[i].first == "nd")
			{
				std::pair<std::string, std::string> valor_node =
					Util::id_node(camiJoaquim.fills[i].second);
				std::cout << valor_node.first << std::endl;
			}
		}
	}
